...
module.exports = class ImageSearchCommand extends Command {
  ...
  async run(message, args, prefix) {
    ...
    if (args) {
      ...
      if (result = result.text.match(/class="thumb"(?:(?!href)[\s\S])+href="([^"]+)/)) message.channel.send(`First result found for query ${args}: ${result[1]}`);
      else message.channel.send(`There is no result for query ${args}`);
